...
const TicketManage: React.FunctionComponent<ITicketManageProps> = (
  ...
  const [checkedListStatus, setCheckedList] = useState(defaultCheckedListStatus)
  const [indeterminate, setIndeterminate] = useState(true)
  const [checkAll, setCheckAll] = useState(false)

  const onChange = (list: any) => {
    setCheckedList(list)
    setIndeterminate(!!list.length && list.length < plainOptionsStatus.length)
    setCheckAll(list.length === plainOptionsStatus.length)
  }

  const onCheckAllChange = (e: any) => {
    setCheckedList(e.target.checked ? plainOptionsStatus : [])
