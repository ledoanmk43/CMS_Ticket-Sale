...
const defaultCheckedListStatus = ['Chưa sử dụng']

const plainOptionsGate: string[] = [
  'Cổng 1',
  'Cổng 2',
  'Cổng 3',
  'Cổng 4',
...
const TicketManage: React.FunctionComponent<ITicketManageProps> = (
  ...
  const [checkAllGate, setCheckAllGate] = useState(false)
  ...
  const onChangeGate = (list: any) => {
    ...
    setIndeterminateGate(!!list.length && list.length < plainOptionsGate.length)
    setCheckAllGate(list.length === plainOptionsGate.length)
  }

  const onCheckAllChangeGate = (e: any) => {
    setCheckedListGate(e.target.checked ? plainOptionsGate : [])
    setIndeterminateGate(false)
    setCheckAllGate(e.target.checked)
  }
  return (
