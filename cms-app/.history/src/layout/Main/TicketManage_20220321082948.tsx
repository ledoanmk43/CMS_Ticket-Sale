...
const plainOptions: string[] = ['Đã sử dụng', 'Chưa sử dụng', 'Hết hạn']
...
const TicketManage: React.FunctionComponent<ITicketManageProps> = (
  ...
  const onChange = (list: any) => {
    setCheckedList(list)
    setIndeterminate(
      !!list.length && list.length < plainOptions.slice(0).length
    )
    setCheckAll(list.length === plainOptions.slice(0).length)
  }
