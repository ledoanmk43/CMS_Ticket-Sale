...
const TicketCheck: React.FunctionComponent<ITicketCheckProps> = (
  ...
  const handleFilter = () => {
    if (checkedStatus !== null) {
      setFilterValue(
        [checkedStatus].map((index: number) => {
          return {
  ...
  console.log('Hello ', checkedStatus)
  useEffect(() => {
    if (debounceSearch) {
      setTableData(
        ...
        setDefaultData(
          props.ticketsData.filter((item: Ticket) => {
            for (let i = 0; i < filterValue.length; i++) {
              if (
                item.checkingStatus.toString() ===
                  filterValue[i].checkingStatus &&
                item.dateUse >= filterValue[i].dateFrom &&
                item.dateUse <= filterValue[i].dateTo
              ) {
                console.log('nồ ', filterValue[i].checkingStatus)
                return item
              }
            }
          })
