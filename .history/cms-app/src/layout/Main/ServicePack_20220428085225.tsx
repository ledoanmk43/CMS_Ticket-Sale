...
const ServicePack: React.FunctionComponent<IServicePackProps> = (
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (packageStatus: boolean) => {
        ...
        let status = ''
        let bg = ''
        if (packageStatus === true && packageStatus !== null) {
          color = '#03AC00'
          status = 'Đang áp dụng'
          bg = '#DEF7E0'
        } else {
  ...
  const headers = [
    { label: 'STT', key: 'id' },
    { label: 'Mã gói', key: 'bookingId' },
    { label: 'Tên gói vé', key: 'ticketId' },
    { label: 'Ngày áp dụng', key: 'status' },
    { label: 'Ngày hết hạn', key: 'dateUse' },
    { label: 'Giá vé (VNĐ/vé)', key: 'dateRelease' },
    { label: 'Giá combo (VNĐ/combo)', key: 'gate' },
    { label: 'Tình trạng', key: 'gate' },
  ]
