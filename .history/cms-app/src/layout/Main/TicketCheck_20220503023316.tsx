...
const TicketCheck: React.FunctionComponent<ITicketCheckProps> = (
  ...
  const handleFilter = () => {
    if (checkedStatus !== '') {
      setFilterValue(
        props.ticketsData.filter((item: Ticket, index: number) => {
          for (let i = 0; i < props.ticketsData.length; i++) {
            if (JSON.stringify(item.checkingStatus) === checkedStatus) {
              return {
                dateFrom: Timestamp.fromDate(dateFrom),
                dateTo: Timestamp.fromDate(dateTo),
                checkingStatus: checkedStatus,
              }
            }
          }
        })
      ...
      setFilterValue(
        props.ticketsData.map((item: Ticket, index: number) => {
          for (let i = 0; i < props.ticketsData.length; i++) {
            if (JSON.stringify(item.checkingStatus) === checkedStatus) {
              return {
                dateFrom: Timestamp.fromDate(dateFrom),
