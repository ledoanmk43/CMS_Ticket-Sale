...
const TicketManage: React.FunctionComponent<ITicketManageProps> = (
  ...
  console.log('lọc nè', filterValue)
          ...
          props.ticketsData.filter((item: Ticket) => {
            for (let i = 0; i < filterValue.length; i++) {
              if (
                ...
                item.dateUse >= Timestamp.fromDate(filterValue[i].dateFrom) &&
                item.dateUse <= Timestamp.fromDate(filterValue[i].dateTo)
              ) {
                return item
              }
              console.log('nồ ' + item.id, item)
            }
          })
        )
      } else {
