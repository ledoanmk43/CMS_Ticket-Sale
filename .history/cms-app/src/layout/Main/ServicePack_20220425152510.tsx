...
const ServicePack: React.FunctionComponent<IServicePackProps> = (
    ...
    },
    {
      ...
      render: (record: any) => {
        return (
          <button
            onClick={() => {
              console.log(record.id, ' before saving', record.packageStatus)
  ...
  const handleEditPack = async () => {
    ...
    console.log(packUpdate.id, 'after saving', packUpdate.packageStatus)
  }
  useEffect(() => {
    if (debounceSearch) {
      setTableData(
          ...
          >
            ...
            <div className='package-input mt-3rem'>
              ...
              <div className='select-status'>
                <Select
                  initialValues={packUpdate.packageStatus}
                  onChange={(e: any) => setPackageStatusEdit(e.value)}
                  style={{
                    maxWidth: '160px',
