          ...
          props.ticketsData.filter((item: Ticket) => {
            for (let i = 0; i < filterValue.length; i++) {
              if (
                ...
                new Date(new Date(item.dateUse).toLocaleDateString('vi-VN')) >=
                  new Date(filterValue[i].dateFrom) &&
                new Date(new Date(item.dateUse).toLocaleDateString('vi-VN')) <=
                  new Date(filterValue[i].dateTo)
              ) {
                return item
              }
            }
          ...
          props.ticketsData.filter((item: Ticket) => {
            for (let i = 0; i < filterValue.length; i++) {
              if (
                ...
                new Date(new Date(item.dateUse).toLocaleDateString('vi-VN')) >=
                  new Date(
                    filterValue[i].dateFrom.toLocaleDateString('vi-VN')
                  ) &&
                new Date(new Date(item.dateUse).toLocaleDateString('vi-VN')) <=
                  new Date(filterValue[i].dateTo.toLocaleDateString('vi-VN'))
              ) {
                return item
              }
            }
