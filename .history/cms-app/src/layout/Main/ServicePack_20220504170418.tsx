...
import { Table, Modal, DatePicker, Select, TimePicker } from 'antd'
import {
  collection,
  getDocs,
  query,
...
const ServicePack: React.FunctionComponent<IServicePackProps> = (
  ...
  const handleAddPack = async () => {
    const docData = {
      ...
      dateBegin: Timestamp.fromDate(dateFromAdd),
      dateEnd: Timestamp.fromDate(
        new Date(
          new Date(dateToAdd).setFullYear(new Date(dateToAdd).getFullYear() + 1)
        )
      ),
    ...
    setPackageStatusAdd(options[0].value)
    setDateFromAdd(new Date())
      ...
      if (!querySnapshot.empty) {
        querySnapshot.forEach((doc) => {
          updateDoc(doc.ref, {
            ...
            dateBegin: Timestamp.fromDate(dateFromUpdate),
            dateEnd: Timestamp.fromDate(
              new Date(
                new Date(dateToUpdate).setFullYear(
                  new Date(dateToUpdate).getFullYear() + 1
                )
              )
            ),
  ...
  //--------------Update
  const [isCalendarFromUpdateOpen, setIsCalendarFromUpdateOpen] =
    useState<boolean>(false)
  const [isCalendarToUpdateOpen, setIsCalendarToUpdateOpen] =
    useState<boolean>(false)
  //Date checking
  const [dateFromUpdate, setDateFromUpdate] = useState<any>(new Date())
  const [dateToUpdate, setDateToUpdate] = useState<any>(new Date())

  //---------------Add
  const [isCalendarFromAddOpen, setIsCalendarFromAddOpen] =
    useState<boolean>(false)
  const [isCalendarToAddOpen, setIsCalendarToAddOpen] = useState<boolean>(false)
  //Date checking
  const [dateFromAdd, setDateFromAdd] = useState<any>(new Date())
  const [dateToAdd, setDateToAdd] = useState<any>(new Date())
  useEffect(() => {
    if (debounceSearch) {
      setTableData(
  ...
  // console.log('dateEnd ne', packUpdate.dateEnd)
  return (
            ...
            >
              <div className='from' style={{ marginRight: '4rem' }}>
                ...
                >
                  <DatePicker
                    onClick={() =>
                      setIsCalendarFromUpdateOpen(!isCalendarFromUpdateOpen)
                    }
                    style={{ position: 'relative' }}
                    value={
                      dateFromUpdate
                        ? moment(new Date(dateFromUpdate), 'DD MMM, YYYY')
                        : moment(new Date(dateFromUpdate), 'DD MMM, YYYY')
                    }
                    defaultValue={moment(
                      new Date(dateFromUpdate),
                      'DD MMM, YYYY'
                    )}
                    ...
                    allowClear={false}
                  />
                  {isCalendarFromUpdateOpen && (
                    <Calendar
                      setIsCalendarOpen={setIsCalendarFromUpdateOpen}
                      setDate={setDateFromUpdate}
                      onClickOutside={() => {
                        setIsCalendarFromUpdateOpen(false)
                      }}
                    />
                  )}
              ...
              <div className='to'>
                ...
                <div style={{ display: 'flex', maxHeight: '50px' }}>
                  <DatePicker
                    onClick={() =>
                      setIsCalendarToUpdateOpen(!isCalendarToUpdateOpen)
                    }
                    style={{ position: 'relative' }}
                    value={
                      dateFromUpdate === new Date()
                        ? moment(new Date(packUpdate.dateBegin), 'DD MMM, YYYY')
                        : moment(new Date(dateToUpdate), 'DD MMM, YYYY')
                    }
                    defaultValue={moment(
                    ...
                    allowClear={false}
                  />
                  {isCalendarToUpdateOpen && (
                    <Calendar
                      setIsCalendarOpen={setIsCalendarToUpdateOpen}
                      setDate={setDateToUpdate}
                      onClickOutside={() => {
                        setIsCalendarToUpdateOpen(false)
                      }}
                    />
                  )}
                  <TimePicker style={{ marginLeft: '8px' }} />
            ...
            >
              <div className='from' style={{ marginRight: '4rem' }}>
                ...
                >
                  <DatePicker
                    onClick={() =>
                      setIsCalendarFromAddOpen(!isCalendarFromAddOpen)
                    }
                    style={{ position: 'relative' }}
                    value={
                      dateFromAdd
                        ? moment(new Date(dateFromAdd), 'DD MMM, YYYY')
                        : moment(new Date(dateFromAdd), 'DD MMM, YYYY')
                    }
                    defaultValue={moment(new Date(dateFromAdd), 'DD MMM, YYYY')}
                    defaultPickerValue={moment(
                      new Date(dateFromAdd),
                      'DD MMM, YYYY'
                    )}
                    format={'DD/MM/YYYY'}
                    allowClear={false}
                  />
                  {isCalendarFromAddOpen && (
                    <Calendar
                      setIsCalendarOpen={setIsCalendarFromAddOpen}
                      setDate={setDateFromAdd}
                      onClickOutside={() => {
                        setIsCalendarFromAddOpen(false)
                      }}
                    />
                  )}
                  <TimePicker style={{ marginLeft: '8px' }} />
              ...
              <div className='to'>
                ...
                <div style={{ display: 'flex', maxHeight: '50px' }}>
                  <DatePicker
                    onClick={() => setIsCalendarToAddOpen(!isCalendarToAddOpen)}
                    style={{ position: 'relative' }}
                    value={
                      dateToAdd
                        ? moment(new Date(dateToAdd), 'DD MMM, YYYY')
                        : moment(new Date(dateToAdd), 'DD MMM, YYYY')
                    }
                    defaultValue={moment(new Date(dateToAdd), 'DD MMM, YYYY')}
                    defaultPickerValue={moment(
                      new Date(dateToAdd),
                      'DD MMM, YYYY'
                    )}
                  ...
                  {isCalendarToAddOpen && (
                    <Calendar
                      setIsCalendarOpen={setIsCalendarToAddOpen}
                      setDate={setDateToAdd}
                      onClickOutside={() => {
                        setIsCalendarToAddOpen(false)
                      }}
                    />
                  )}
                  <TimePicker style={{ marginLeft: '8px' }} />
