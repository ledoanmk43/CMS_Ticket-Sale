...
const TicketManage: React.FunctionComponent<ITicketManageProps> = (
  ...
  const headers = [
    ...
    { label: 'Ngày xuất vé', key: 'dateRealse' },
    { label: 'Cổng check-in', key: 'gate' },
  ]

  const [dataCSV, setDataCSV] = useState<any>([
    { label: 'STT', key: '' },
    { label: 'Booking Code', key: '' },
    { label: 'Số vé', key: 'ticketId' },
    { label: 'Tình trạng sử dụng', key: '' },
    { label: 'Ngày sử dụng', key: '' },
    { label: 'Ngày xuất vé', key: '' },
    { label: 'Cổng check-in', key: '' },
  ])
  ...
  const handleCSV = () => {
    ...
    // { label: 'Cổng check-in', key: 'gate' },
    //]
    const data: Ticket[] = debounceSearch.length ? tableData : defaultData
    setDataCSV(
      data.map((data: any) => ({
        label: dataCSV.forEach((item: any) => item.label),
        // key: data.map((data: Ticket) => {
  ...
  }, [debounceSearch, props.ticketsData, filterValue, dataCSV])
