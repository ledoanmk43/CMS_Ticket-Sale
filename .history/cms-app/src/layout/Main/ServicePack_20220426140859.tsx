  ...
  const handleEditPack = async () => {
    try {
      ...
      const dataUpdate = {
        id: packUpdate.id,
        packageId: 'ALT20210501',
        packageName: packageNameEdit ? packageNameEdit : packUpdate.packageName,
        dateBegin: Timestamp.fromDate(new Date()),
        dateEnd: Timestamp.fromDate(new Date()),
        ticketPrice: ticketPriceEdit ? ticketPriceEdit : packUpdate.ticketPrice,
        comboPrice: comboPriceEdit ? comboPriceEdit : packUpdate.comboPrice,
        packageStatus: packageStatusEdit,
      }
      if (!querySnapshot.empty) {
        querySnapshot.forEach((doc) => {
          updateDoc(doc.ref, dataUpdate)
        })
        setPackUpdate(dataUpdate)
        setPackageNameEdit(null)
        setTicketPriceEdit(null)
        setComboPriceEdit(null)
        setPackageStatusEdit(packageStatusEdit)
      }
    } catch (e) {
      console.log(e)
    }
