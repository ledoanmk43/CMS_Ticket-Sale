...
const ServicePack: React.FunctionComponent<IServicePackProps> = (
    ...
    },
    {
      ...
      render: (record: any) => {
        return (
          <button
            onClick={() => {
              console.log('before saving', record.packageName)
  ...
  const handleEditPack = async () => {
      ...
      if (!querySnapshot.empty) {
        querySnapshot.forEach((doc) => {
          updateDoc(doc.ref, {
            id: packUpdate.id,
            packageId: 'ALT20210501',
            packageName: packUpdate.packageNameEdit
              ? packUpdate.packageNameEdit
              : packageNameEdit,
            dateBegin: Timestamp.fromDate(new Date()),
            dateEnd: Timestamp.fromDate(new Date()),
            packageStatus: packUpdate.packageStatusEdit
              ? packUpdate.packageStatusEdit
              : packageStatusEdit,

            ticketPrice: packUpdate.ticketPriceEdit
              ? packUpdate.ticketPriceEdit
              : ticketPriceEdit,
            comboPrice: packUpdate.comboPriceEdit
              ? packUpdate.comboPriceEdit
              : comboPriceEdit,
          })
        })
      }
    ...
    console.log('after saving', packUpdate.packageName)
  }
  useEffect(() => {
    if (debounceSearch) {
      setTableData(
