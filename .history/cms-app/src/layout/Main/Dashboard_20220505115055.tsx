...
import { Packages, Ticket } from './Home'
...
const Dashboard: React.FunctionComponent<IDashboardProps> = (
  ...
  const hanleDashboard = () => {
    setDataArea(
      props.packagesData.map((item: any) => {
        return {
          dateUse: item.dateBegin.toDate().getMonth(),
          price: item.comboPrice / 10,
        }
      })
    )
  }
  ...
  useEffect(() => {
    ...
    hanleDashboard()
  }, [props.ticketsData, props.packagesData])
  console.log('dataArea nè', props.packagesData)
