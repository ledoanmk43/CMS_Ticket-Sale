...
const TicketManage: React.FunctionComponent<ITicketManageProps> = (
  ...
  // Status Checking
  const [checkedStatus, setCheckedStatus] = useState<any>(options[2].value)
          ...
          props.ticketsData.filter((item: Ticket) => {
            for (let i = 0; i < filterValue.length; i++) {
              if (
                ...
                filterValue[i].dateFrom <=
                  new Date(item.dateUse) <=
                  filterValue[i].dateTo
              ) {
                console.log(
                  'item.dateUse ++++ nè',
                  new Date(item.dateUse).toLocaleDateString('en-GB')
                )
                return item
              }
            }
          ...
          props.ticketsData.filter((item: Ticket) => {
            for (let i = 0; i < filterValue.length; i++) {
              if (
                ...
                console.log('item.dateUse ---- nè', new Date(item.dateUse))
                return item
              }
            }
          })
  ...
  // console.log('dateFrom nè', dateFrom)
  // console.log('dateTo nè', dateTo)
