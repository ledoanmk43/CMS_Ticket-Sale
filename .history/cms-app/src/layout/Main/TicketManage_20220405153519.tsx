...
const changeStatus = async () => {
  console.log('clicked')
  try {
    const ticketsCollectionRef: any = collection(db, 'tickets')
    await runTransaction(db, async (transaction) => {
      const tickets: any = await transaction.get(ticketsCollectionRef)
      console.log(tickets.data().status)

      const newStatus = (tickets.data().status = 'used')
      transaction.update(ticketsCollectionRef, {
        status: newStatus,
      })
    })
...
const menu = (
  <Menu className='menu'>
    <Menu.Item>
      <div ref=n'oreferrer' onClick={changeStatus}>Sử dụng vé</div>
    </Menu.Item>
    <Menu.Item>
