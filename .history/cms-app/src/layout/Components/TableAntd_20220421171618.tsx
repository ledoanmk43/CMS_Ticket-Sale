...
const TableAntd: React.FunctionComponent<ITableAntdProps> = (
  ...
  useEffect(() => {
    ...
    if (props.filterValue) {
            ...
            for (let i = 0; i < props.filterValue.length; i++) {
              if (
                JSON.stringify(item.gate) ===
                  JSON.stringify(props.filterValue[i].gate) &&
                JSON.stringify(item.status) ===
                  JSON.stringify(props.filterValue[i].status)
              ) {
                return item
              }
            }
      ...
      } else if (props.filterValue.status == undefined) {
        setDefaultData(
          props.ticketsData.filter((item: Ticket) => {
            for (let i = 0; i < props.filterValue.length; i++) {
              if (
                JSON.stringify(item.gate) ===
                JSON.stringify(props.filterValue[i])
              ) {
                return item
              }
            }
  ...
  }, [debounceSearch, props.ticketsData, props.filterValue])
