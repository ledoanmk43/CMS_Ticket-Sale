...
const TableAntd: React.FunctionComponent<ITableAntdProps> = (
  ...
  useEffect(() => {
    ...
    if (props.checkedListGate.length >= 1) {
      setDefaultData(
        props.ticketsData.filter(function (item: Ticket) {
          for (var i = 0; i < props.checkedListGate.length; i++) {
            if (
              JSON.stringify(item.RepairDate) ==
                JSON.stringify(failedRepairs[i].RepairDate) &&
              JSON.stringify(item.car) ==
                JSON.stringify(failedRepairs[i].car) &&
              JSON.stringify(item.Model) ==
                JSON.stringify(failedRepairs[i].Model)
            ) {
              return item
            }
          }
