...
const TableAntd: React.FunctionComponent<ITableAntdProps> = (
  ...
  const debounceSearch = useDebounce(props.searchValue, 400)
  ...
  const [tableData, setTableData] = useState<Ticket | undefined | any>()
  useEffect(() => {
    if (debounceSearch) {
      return () => {
        setTableData(
          props.ticketsData.filter(
            (item: Ticket) => item.ticketId === debounceSearch
          )
        )
  ...
  return (
    <Table
      ...
      rowKey='id'
      columns={columns}
      dataSource={props.debounceSearch.length ? tableData : props.ticketsData}
      pagination={{
        showTitle: false,
