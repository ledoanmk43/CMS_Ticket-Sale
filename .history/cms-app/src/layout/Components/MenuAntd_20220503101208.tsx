...
const MenuAntd: React.FunctionComponent<IMenuAntdProps> = (
  ...
  const [isCarlendarOpen, setIsCarlendarOpen] = useState<boolean>(false)
  ...
  return (
    <Menu className='menu'>
      <Menu.Item onClick={changeStatus}>Sử dụng vé</Menu.Item>
      <Menu.Item
        onClick={() => {
      ...
      {modalChangeDate && (
        ...
        >
          ...
          <div className='flex row change-date'>
            ...
            <DatePicker
              onClick={() => setIsCarlendarOpen(!isCarlendarOpen)}
            ...
            {isCarlendarOpen && (
              <Calendar
                setIsCarlendarOpen={setIsCarlendarOpen}
